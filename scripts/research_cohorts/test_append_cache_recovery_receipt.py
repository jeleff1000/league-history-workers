...
def test_append_records_has_po_signal_candidate_field(tmp_path):
    fields = [
        "record_type", "receipt_run_id", "canonical_cache_key", "receipt_json_sha256",
        "artifact_id", "artifact", "workflow_run_id", "dispositions",
        "candidate_delta_rows", "candidate_fields", "candidate_file_count", "candidate_files",
    ...
    readback = tmp_path / "readback.json"
    readback.write_text(json.dumps({
        "canonical_cache_key": "key", "candidate_run_id": "1", "candidate_rows": 1,
        "candidate_artifacts": 1, "source_artifact_id": "source",
        "candidate_fields": ["has_po_signal"], "matched_player_rows": 1,
        "unmatched_candidate_keys": 0, "remaining_source_backed_nulls": {"has_po_signal": 0},
        "schema_unchanged": True, "player_rows_unchanged": True, "ops_unchanged": True,
    ...
    with out.open(newline="") as handle:
        row = list(csv.DictReader(handle))[0]
    assert row["candidate_fields"] == "has_po_signal"
