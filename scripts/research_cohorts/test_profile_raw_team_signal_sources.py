...
def test_unmatched_source_manager_reports_no_canonical_player_recipient(tmp_path):
    """An unmatched manager must be classified instead of left as a generic gap."""
    base = tmp_path / "canonical.duckdb"
    con = duckdb.connect(str(base))
    ...
    con.execute("""
        COPY (
          SELECT 'league'::VARCHAR AS db_name, 2024::INTEGER AS year,
                 1::INTEGER AS week, 'sleeper'::VARCHAR AS platform,
                 'source-team-c'::VARCHAR AS team_key,
                 'Carol'::VARCHAR AS manager, 'Comets'::VARCHAR AS team_name
        ) TO ? (FORMAT PARQUET)
    """, [str(source)])
    con.close()
    ...
    assert result["unresolved_identity_reasons"] == [{
        "artifact_id": 1,
