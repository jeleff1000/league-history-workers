...
TEAM_KEY = ["db_name", "year", "week", "manager"]
SUPPORTED = {
...
def build(*, base: Path, manifest: Path, out: Path, report: Path) -> dict:
    ...
    for entry in entries:
        ...
        if not required <= raw_columns:
            raise ValueError(f"{artifact_id}: source missing team key {sorted(required - raw_columns)}")
        terms = [
            f"{artifact_id}::BIGINT AS artifact_id",
            f"CAST({_q('db_name')} AS VARCHAR) AS {_q('db_name')}",
            f"TRY_CAST({_q('year')} AS INTEGER) AS {_q('year')}",
            f"TRY_CAST({_q('week')} AS INTEGER) AS {_q('week')}",
            f"NULLIF(TRIM(CAST({_q('manager')} AS VARCHAR)), '') AS {_q('manager')}",
        ]
    ...
    con.execute(f"""
        COPY (
          SELECT player_rowid, {', '.join(_q(key) for key in KEY)}, source_artifact_ids,
    ...
    duplicate_delta_keys = int(con.execute(f"SELECT COUNT(*) FROM (SELECT player_rowid FROM read_parquet({_lit(out)}) GROUP BY ALL HAVING COUNT(*) > 1)").fetchone()[0])
    ...
    result = {
        ...
        "source_team_groups_missing_manager": missing_manager,
        "unmatched_source_team_groups": unmatched,
        "unmatched_canonical_keys": unmatched,
        "canonical_duplicate_exact_keys": 0,
