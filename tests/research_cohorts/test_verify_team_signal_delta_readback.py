...
def test_uses_saved_apply_receipt_when_legacy_delta_lacks_canonical_prestate(tmp_path: Path) -> None:
    base = tmp_path / "base.duckdb"
    con = duckdb.connect(str(base))
    con.execute("CREATE SCHEMA public")
    con.execute("""CREATE TABLE public.player_fantasy AS SELECT
      'league'::VARCHAR db_name, 2020::INTEGER AS "year", 7::INTEGER AS "week",
      'nfl-1'::VARCHAR NFL_player_id, 'manager'::VARCHAR manager,
      'team'::VARCHAR team_key, 'team'::VARCHAR team_name, 'mfl'::VARCHAR platform,
      1::INTEGER win, NULL::INTEGER loss, NULL::INTEGER tie, 10.0::DOUBLE team_points,
      1::INTEGER is_playoffs, NULL::INTEGER champion, NULL::INTEGER final_playoff_seed,
      NULL::INTEGER made_playoffs""")
    schema = con.execute("DESCRIBE public.player_fantasy").fetchall()
    con.execute("""CREATE TABLE d AS SELECT
      'league'::VARCHAR db_name, 2020::INTEGER AS "year", 7::INTEGER AS "week",
      'nfl-1'::VARCHAR NFL_player_id, 'manager'::VARCHAR manager,
      'team'::VARCHAR team_key, 'team'::VARCHAR team_name, 'mfl'::VARCHAR platform,
      1::INTEGER source_win, NULL::INTEGER source_loss, NULL::INTEGER source_tie,
      12.0::DOUBLE source_team_points, 1::INTEGER source_playoffs,
      NULL::INTEGER source_champion, NULL::INTEGER source_final_playoff_seed,
      NULL::INTEGER source_made_playoffs""")
    delta = tmp_path / "delta.parquet"
    con.execute("COPY d TO ? (FORMAT PARQUET)", [str(delta)])
    con.close()
    before = tmp_path / "before.json"
    ...
    apply.write_text(json.dumps({"improvements_by_field":{"is_playoffs":1}, "replacements_by_field":{"win":1}}), encoding="utf-8")
    output = tmp_path / "result.json"

    result = subprocess.run(
        [sys.executable, str(SCRIPT), "--base", str(base), "--delta", str(delta), "--before", str(before), "--ops", str(ops), "--ops-before", str(ops_before), "--apply-report", str(apply), "--out", str(output)],
        text=True, capture_output=True, check=False,
    )

    assert result.returncode == 0, result.stderr
    report = json.loads(output.read_text())
    assert report["readback_proof_mode"] == "legacy_apply_receipt_minimum_match"
    assert report["authorized_cells_by_field"] == {"is_playoffs": 1, "win": 1}
